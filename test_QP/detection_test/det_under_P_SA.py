...
tau_result_frame = pd.DataFrame(result_array)
tau_result_frame.to_csv('det_SA_L1_ded_12_03_21_M10M12.csv')
    ...
    for del_evg_te in DEL_AVG_ARRAY_DED_TE:
        for ro_mal in RO_MAL_ARRAY:
            ...
            if (len(fa_C) > 1):
                ...
                T_btw_FA += 365 - fa_C[len(fa_C) - 1]
                efa_QC = T_btw_FA / (len(fa_C) - 1)
            object_c = {"epsilon": row.epsilon, 'del_avg_te': del_evg_te, 'ro_mal': ro_mal, 'efa': efa_QC,
                        'ro_max': row.ro_max,
                        'type': 'ded', 'tau_max': row.tau_max, 'tau_min': row.tau_min,
                        'day_detected': first_detected_org_c}
            result_array.append(object_c)
...
tau_result_frame = pd.DataFrame(result_array)
tau_result_frame.to_csv('det_SA_L2_ded_12_03_21_M10M12.csv')
