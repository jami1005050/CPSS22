...
f = open('../result/standard_limit/poison.json')
    ...
    for key2 in poisoning_std_limits[key1].keys(): # romax
        ...
        for key3 in poisoning_std_limits[key1][key2].keys(): #epsilon
            ...
            for del_avg in DEL_AVG_ARRAY_DED:
                ...
                result[key1][key2][key3][del_avg] = {}
                tier1_anomaly,tier2_for_org,first_detected_org,false_alarm = testing_tau(testing_residualM4M6,
                                                                                         poisoning_std_limits[key1][key2][key3]['tau_max'],
                                                                                         poisoning_std_limits[key1][key2][key3]['tau_min'])
                result[key1][key2][key3][del_avg] ['tier1'] = tier1_anomaly
                result[key1][key2][key3][del_avg] ['tier2'] = tier2_for_org
                result[key1][key2][key3][del_avg] ['first_detected'] = first_detected_org
                result[key1][key2][key3][del_avg] ['false_alarm'] = false_alarm


with open("../result/detection/detection_poisoned_Q.json", "w") as outfile:
