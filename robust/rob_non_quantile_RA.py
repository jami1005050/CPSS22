...
import pandas as pd
import json
from utility.constant import BETA_ARR, DEL_AVG_ARRAY_DED
...
for del_avg in DEL_AVG_ARRAY_DED:
    ...
    ruc_frame = pd.read_csv('random_attack_TR_RUC/training_RUC_del_'+str(del_avg)+'csv')
    keys_TR = ['2014','2015']
    ...
    for beta in BETA_ARR:
        train_t_max, train_t_max_loss_list, train_t_max_list = getattr(utils, 'calculate_t_max_' + loss_function)(
                                                                       ruc_frame, keys_TR,
                                                                       tau_range=[.0, max_candidate, .0025], b=beta)
        train_t_min, train_t_min_loss_list, train_t_min_list = getattr(utils, 'calculate_t_min_' + loss_function)(
                                                                       ruc_frame,  keys_TR,
                                                                       tau_range=[min_candidate, .0, .0025], b=beta)
        rob_res_benign[del_avg][beta] = {'tau_max':train_t_max,'tau_min':train_t_min}


with open("../test_C/robust_C_non_Q_RA.json", "w") as outfile:
