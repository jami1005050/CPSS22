...
dataframe_ded_Q_C = pd.read_csv('../../robust/tau_generation_OPT_BETA/RA_tau_OB_C_Q_ded.csv')
dataframe_add_NQ_C = pd.read_csv('../../robust/tau_generation_OPT_BETA/RA_tau_OB_C_NQ_add.csv')
dataframe_ded_NQ_C = pd.read_csv('../../robust/tau_generation_OPT_BETA/RA_tau_OB_C_NQ_ded.csv')
...
result_array = []
for index, row in dataframe_ded_Q_C.iterrows():
...
tau_result_frame = pd.DataFrame(result_array)
tau_result_frame.to_csv('det_RA_Q_C_ded.csv')
...
result_array = []
for index, row in dataframe_ded_NQ_C.iterrows():
    for del_evg_te in DEL_AVG_ARRAY_DED_TE:
        file_exists = os.path.exists(
            '../../data/test_RUC_OPT_BETA/deductive/Test_RUC_TR' + str(int(row.del_avg)) + '_TE_' +
                                str(del_evg_te) + '_RO_' + str(row.ro_mal) + 'dedM10M12.csv')
...
tau_result_frame = pd.DataFrame(result_array)
tau_result_frame.to_csv('det_RA_NQ_C_ded.csv')